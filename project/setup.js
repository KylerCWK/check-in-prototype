...
`;
    ...
    let mongodbUri = existingEnv.MONGODB_URI || process.env.MONGODB_URI;
    if (!mongodbUri) {
      console.error('❌ ERROR: MONGODB_URI environment variable is required');
      process.exit(1);
    }

    // Remove any malformed retryWrites (e.g., retryWrites with no value)
    mongodbUri = mongodbUri.replace(/retryWrites([=&][^&]*)?/g, '');

    // Clean up stray ampersands or question marks left behind
    mongodbUri = mongodbUri.replace(/[?&]+$/, '');

    // Add retryWrites=true properly
    const separator = mongodbUri.includes('?') ? '&' : '?';
    mongodbUri += `${separator}retryWrites=true`;
